...
Wall_Select() {
    ...
    if [ -n "$HYPRLAND_INSTANCE_SIGNATURE" ]; then
        ...
        mon_scale=$(jq '.[] | select(.focused==true) | .scale' <<<"$mon_data")
    fi
    mon_x_res=${mon_x_res:-1920}
    mon_scale=${mon_scale:-1}
    mon_x_res=$(awk "BEGIN{printf \"%d\", $mon_x_res * 100 / $mon_scale}")
    ...
    listview{columns:${ROFI_WALLPAPER_COLUMN_COUNT:-${col_count:-3}};lines:0;fixed-height:false;spacing:5em;}
