...
load_hypr_variables() {
    ...
    if command -v hyq &>/dev/null; then
        ...
        # Fallback: parse hyprland config with grep
        GTK_THEME=$(grep -o '^\s*gtk-theme\s*=\s*"\([^"]*\)"' "$hypr_file" | head -1 | cut -d'"' -f2)
        ICON_THEME=$(grep -o '^\s*icon-theme\s*=\s*"\([^"]*\)"' "$hypr_file" | head -1 | cut -d'"' -f2)
        CURSOR_THEME=$(grep -o '^\s*cursor-theme\s*=\s*"\([^"]*\)"' "$hypr_file" | head -1 | cut -d'"' -f2)
        CURSOR_SIZE=$(grep -o '^\s*cursor-size\s*=\s*[0-9]*' "$hypr_file" | head -1 | grep -o '[0-9]*')
        FONT=$(grep -o '^\s*font-name\s*=\s*"\([^"]*\)"' "$hypr_file" | head -1 | cut -d'"' -f2)
        FONT_SIZE=$(grep -o '^\s*font-size\s*=\s*[0-9]*' "$hypr_file" | head -1 | grep -o '[0-9]*')
        DOCUMENT_FONT=$(grep -o '^\s*document-font-name\s*=\s*"\([^"]*\)"' "$hypr_file" | head -1 | cut -d'"' -f2)
        DOCUMENT_FONT_SIZE=$(grep -o '^\s*document-font-size\s*=\s*[0-9]*' "$hypr_file" | head -1 | grep -o '[0-9]*')
        MONOSPACE_FONT=$(grep -o '^\s*monospace-font-name\s*=\s*"\([^"]*\)"' "$hypr_file" | head -1 | cut -d'"' -f2)
        MONOSPACE_FONT_SIZE=$(grep -o '^\s*monospace-font-size\s*=\s*[0-9]*' "$hypr_file" | head -1 | grep -o '[0-9]*')
    fi
}
sanitize_hypr_theme() {
    input_file="$1"
